...
static void NONRET cf_err(char *msg)
{
    die("Error in config file, line %d: %s", line_number, msg);
}

static char *cf_string(char *val)
{
    return xstrdup(val);
}

static int cf_int(char *val)
{
    char *end;
    errno = 0;
...
static void cf_entry_toplevel(char *key, char *val)
{
...
static void cf_entry_compound(char *key, char *subkey, char *val)
{
...
static void cf_entry(char *key, char *val)
{
    char *dot = strchr(key, '.');
    if (!dot)
